...
-- AUDITORIA PARA VER LOS REGISTROS 
...
-- HACER UN PROCEDURE PARA CURSOS QUE ENSEÑA UN PROFESOR
DROP PROCEDURE IF EXISTS obtener_cursos_por_profesor;
DELIMITER $$
CREATE PROCEDURE obtener_cursos_por_profesor (
    IN profesor_DNI INT
)
BEGIN
    -- Selecciona los cursos que enseña el profesor con el DNI proporcionado
    ...
    JOIN modulo m ON phm.modulo_código = m.código
    JOIN curso c ON m.código = c.modulo_código
    WHERE p.DNI = profesor_DNI;
END$$
DELIMITER ;

CALL obtener_cursos_por_profesor(12345678);
